        ...
        public static class KillButtonHandler
        {
            public static void KillButtonUpdate(Icenberg __gInstance, HudManager __instance)
            {
                ...
                __instance.KillButton.SetCoolDown(
                    CustomGameOptions.IcenbergKillCooldown -
            ...
            public static void FreezeAllButtonUpdate(Icenberg __gInstance, HudManager __instance)
            {
                ...
                if (__gInstance.FreezeAllButtonUsed)
                {
                    if (__gInstance.FreezeAllButton != null)
                    {
                        ...
                        __gInstance.FreezeAllButton.graphic.enabled = false;
                        __gInstance.FreezeAllButton.SetTarget(null);
                        __gInstance.FreezeAllButton.transform.position = new Vector3(-999f, -999f, 0f);
                    }
                    return;
                }
                ...
                if (__gInstance.FreezeAllButton == null)
                {
                    ...
                    __gInstance.FreezeAllButton.name = "FreezeAllButton";
                }

                bool shouldBeVisible = (__instance.UseButton.isActiveAndEnabled || __instance.PetButton.isActiveAndEnabled)
                    && !MeetingHud.Instance && !__gInstance.Player.Data.IsDead
                    && AmongUsClient.Instance.GameState == InnerNetClient.GameStates.Started;

                __gInstance.FreezeAllButton.gameObject.SetActive(shouldBeVisible);
                if (shouldBeVisible)
                {
                    ...
                    __gInstance.FreezeAllButton.SetCoolDown(
                        CustomGameOptions.FreezeCooldown - (float)(DateTime.UtcNow - __gInstance.LastFreeze).TotalSeconds,
                        CustomGameOptions.FreezeCooldown
                    );
                }
            }
